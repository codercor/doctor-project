...
import { Star, SendSharp, Refresh, ArrowBackIos } from "@mui/icons-material";
...
const ChatMessage = ({ isMe = true, message }: ChatMessageProps) => {
    ...
    return <div className={classNames("flex p-[20px] h-fit mt-[20px] w-[80%] ", {
        ...
        {isMe && <div className="flex text-[white] items-center justify-center md:min-w-[60px] min-w-[50px] h-[50px] md:h-[60px] rounded-full bg-secondary">
            <Star />
        </div>}
        ...
        {!isMe && <div className="flex  text-[white] items-center justify-center  md:min-w-[60px] min-w-[50px] h-[50px] md:h-[60px] rounded-full bg-secondary-flat">
            <Star />
        </div>}
    </div>
}
const ChatBox = ({ back }: { back: () => void }) => {
    ...
    return <div className="px-[30px] md:relative absolute h-[70%]  w-full md:h-full flex flex-col bg-[#F9FBFC]">
        {SelectedChatLineId && <div onClick={() => {
            getChatLineMessages(SelectedChatLineId)
        }} className="w-[40px] hover:shadow-md shadow-xl transition-all cursor-pointer grid place-content-center rounded-[5px_20px] absolute top-0 right-1 h-[40px] bg-secondary text-[white]">
            <Refresh />
        </div>}
        {SelectedChatLineId && <div onClick={() => {
            back()
        }} className="w-[40px] hover:shadow-md shadow-xl pl-2 transition-all cursor-pointer grid md:hidden place-content-center rounded-[5px_20px] absolute top-0 left-1 h-[40px] bg-secondary text-[white]">
            <ArrowBackIos className="scale-90" />
        </div>}
