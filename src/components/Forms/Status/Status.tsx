...
export type StepStatusType = 'confirmed' | 'rejected' | 'pending' | 'inReview' | 'Gönderildi' | 'Bekliyor' | 'Reddedildi'
const StepStatus = ({ status }: { status: StepStatusType }) => {
    const statusColorClasses = classNames('w-[110px] rounded-md h-[30px] flex items-center justify-between', {
        "bg-[#F6FFED] border-[1px] border-[#B7EB8F] text-[#389E0D]": status === "confirmed" || status === "Gönderildi",
    ...
    const text = status === "confirmed" ? "Onaylandı" : (status === "rejected" || status === "Reddedildi") ? "Reddedildi" : status === "pending" ? "Beklemede" : status == "Bekliyor" ? "Bekliyor" : status == "Gönderildi" ? "Gönderildi" : "inceleme" // status === "inReview" ?
