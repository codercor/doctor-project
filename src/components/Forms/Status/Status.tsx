...
export type StepStatusType = 'confirmed' | 'rejected' | 'pending' | 'inReview' | 'Gönderildi' | 'Bekliyor'
const StepStatus = ({ status }: { status: StepStatusType }) => {
    ...
    const text = status === "confirmed" ? "Onaylandı" : status === "rejected" ? "Reddedildi" : status === "pending" ? "Beklemede" : status == "Bekliyor" ? "Bekliyor" : status == "Gönderildi" ? "Gönderildi" : "inceleme" // status === "inReview" ?
