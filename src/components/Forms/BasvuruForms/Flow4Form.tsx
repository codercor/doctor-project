    ...
    return (
        <Formik
            ...
            onSubmit={(values) => {
                ...
                let flow3DoneKey = flow3DataKey + "-done";
                let flow3Done = JSON.parse(localStorage.getItem(flow3DoneKey) as string);
                ...
                if (!flow3Data || !flow3Done) {
                    toast("Lütfen önce 2. adımı tamamlayınız.")
                }
                if (flow2Data && flow3Data && flow3Done) {
                    sendForm({ step: 2, values: flow2Data, userId: UserId }).then(() => {
                        ...
                        localStorage.removeItem(flow3DoneKey)
