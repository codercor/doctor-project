...
const FormStep = (props: FormStepType) => {
    ...
    return <div onClick={props?.onClick} className={
        classNames("w-full cursor-pointer h-full border-[#DDDCDC] border-[1px] px-[24px] py-[18px]", {
            "bg-[#F9F9F9]": !props.isActive,
            "bg-[#E8F3F4] border-[#B3D3D6]": props.isActive,
        })}>
        <div className="flex justify-start items-center leading-none">
            <h1 className="text-[#9D9D9D] font-nexa-bold text-[36px] "> {props.stepNumber} </h1>
            {props.status && <span className="ml-[20px]"><StepStatus status={props.status} /></span>}
            {props.isLocked && <Lock fontSize="small" className="text-[#9D9D9D]" />}
        </div>
        <p className="text-[#9D9D9D] text-[18px] font-nexa-regular"> {props.stepName} </p>
    </div >
}
